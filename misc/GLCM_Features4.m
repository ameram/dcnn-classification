...
size_glcm_1 = size(glcmin,1);
size_glcm_2 = size(glcmin,2);
size_glcm_3 = size(glcmin,3);
...
for k = 1:size_glcm_3
    ...
    glcm_sum(k) = sum(sum(glcmin(:,:,k)));
    glcmin(:,:,k) = glcmin(:,:,k)./glcm_sum(k); % Normalize each glcm
    glcm_mean(k) = mean2(glcmin(:,:,k)); % compute mean after norm
    glcm_var(k)  = (std2(glcmin(:,:,k)))^2;
    
    for i = 1:size_glcm_1
        
        for j = 1:size_glcm_2
            p_x(i,k) = p_x(i,k) + glcmin(i,j,k); 
            p_y(i,k) = p_y(i,k) + glcmin(j,i,k); % taking i for j and j for i
            %if (ismember((i + j),[2:2*size_glcm_1])) 
                p_xplusy((i+j)-1,k) = p_xplusy((i+j)-1,k) + glcmin(i,j,k);
            %end
...
for k = 1:size_glcm_3 % number glcms
    ...
    out.contr(k) = sum(sum(mul_contr.*glcmin(:,:,k)));
    out.dissi(k) = sum(sum(mul_dissi.*glcmin(:,:,k)));
    out.energ(k) = sum(sum(glcmin(:,:,k).^2));
    out.entro(k) = - sum(sum((glcmin(:,:,k).*log(glcmin(:,:,k) + eps))));
    out.homom(k) = sum(sum((glcmin(:,:,k)./( 1 + mul_dissi))));
    out.homop(k) = sum(sum((glcmin(:,:,k)./( 1 + mul_contr))));
    ...
    out.sosvh(k) = sum(sum(glcmin(:,:,k).*((i_matrix - glcm_mean(k)).^2)));
    out.indnc(k) = sum(sum(glcmin(:,:,k)./( 1 + (mul_dissi./size_glcm_1) )));
    out.idmnc(k) = sum(sum(glcmin(:,:,k)./( 1 + (mul_contr./(size_glcm_1^2)))));
    out.maxpr(k) = max(max(glcmin(:,:,k)));
    
    u_x(k)       = sum(sum(i_matrix.*glcmin(:,:,k))); 
    u_y(k)       = sum(sum(j_matrix.*glcmin(:,:,k))); 
    ...
    s_x(k)  = (sum(sum( ((i_matrix - u_x(k)).^2).*glcmin(:,:,k) )))^0.5;
    s_y(k)  = (sum(sum( ((j_matrix - u_y(k)).^2).*glcmin(:,:,k) )))^0.5;
    
   corp(k) = sum(sum((i_matrix.*j_matrix.*glcmin(:,:,k))));
   corm(k) = sum(sum(((i_matrix - u_x(k)).*(j_matrix - u_y(k)).*glcmin(:,:,k)))); 
